...
import { queryMandatoryList, deleteRecMandatory, openRecMandatory, closeRecMandatory } from '@/api/valuation/rec-mandatory.js'
export default {
  ...
  methods: {
    ...
    changeStatus(status, id) {
      if (status === '02') {
        closeRecMandatory(id).then(response => {
          this.loadTable()
        })
      } else if (status === '03') {
        openRecMandatory(id).then(response => {
          this.loadTable()
        })
      }
    },
